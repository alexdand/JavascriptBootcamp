...
function extractValue(arr, key) {
    return arr.reduce((current, nextValue) => {
        current.push(nextValue[key]);
        return current;
    }, []);
}
var arr = [{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}]
let output = extractValue(arr, 'name');
console.log(output);
...
function vowelCount(str) {
    return str.toLowerCase().split("").reduce((obj, nextValue) => {
        if ("aeiou".includes(nextValue))
            nextValue in obj ? obj[nextValue]++ : obj[nextValue]=1;
        return obj;
    }, {});
}
...
var arr = [{name: 'Elie'}, {name: 'Tim'}, {name: 'Matt'}, {name: 'Colt'}];
let result = addKeyAndValue(arr, 'title', 'Instructor');
...
function partition(arr, callback) {
    return arr.reduce((output, value) => {
        if (callback(value))
            output[0].push(value);
        else
            output[1].push(value);
